...
rm(list = ls()[! ls() %in% c("df_pr_cr2", "df_tm_cr2", "df_pr_e5", "df_tm_e5")])
...
df_pr_e5  <- df_pr_e5 %>% filter(format(date, "%Y") %in% Agnos)
df_tm_e5  <- df_tm_e5 %>% filter(format(date, "%Y") %in% Agnos)
...
gg_dif <- ggplot(data = df_dif_clim, aes(x = tm, y = pr, shape = cuenca, colour = elevmean)) +
  ...
  labs(x = "Temperatura (°C)", y = "Precipitación (%)", shape = "Cuenca",
       colour = paste0("Elevación", "\n", "(m.s.n.m.)"), title = "Diferencia ERA5 - CR2MET") +
    guides(shape = guide_legend(order = 1)) #+
...
ggsave("gg_Climat_EscalaCuenca.png", gg, height = 3.6, width = 8.2)
rm(gg_cr2, gg_dif, gg)
...
## Guarda
gg <- ggarrange(gg_cr2, gg_dif, common.legend = T, legend = "right")
ggsave("gg_Climat_EscalaCuenca_Agnos.png", gg, height = 3.6, width = 8.2)
rm(gg_cr2, gg_dif, gg)
